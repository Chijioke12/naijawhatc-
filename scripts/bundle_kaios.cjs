...
const OUTPUT_ZIP = path.join(__dirname, '../naijawhot.zip');
const DIRECT_APP_ZIP = path.join(__dirname, '../application.zip');
...
appZip.writeZip(DIRECT_APP_ZIP);
const appZipBuffer = appZip.toBuffer();

console.log('Creating naijawhot.zip (OmniSD package)...');
const omniZip = new AdmZip();

// 1. Add application.zip
omniZip.addFile('application.zip', appZipBuffer);

// 2. Add update.webapp (empty file as per OmniSD spec)
...
console.log(`Successfully created ${OUTPUT_ZIP} and ${DIRECT_APP_ZIP} !`);
